jQuery(document).ready(function($){
	...
	$("[id^='siteseo-toggle-meta-']").on('click', function(){
		...
		if($(this).hasClass('active')){
			...
			$(this).closest('.siteseo-toggle-cnt').find('.siteseo-suboption-toggle').val('1');
		} else{
			$('#toggle_state_posts').text(' Click to show any SEO metaboxes / columns for this post type');
			$(this).closest('.siteseo-toggle-cnt').find('.siteseo-suboption-toggle').val('0');
		}
	});
	...
	$('#siteseo-submit-urls-button').on('click', function(e){
		...
		$.ajax({
			...
			data:{
				...
				nonce: siteseoAdminAjax.nonce,
				search_engine: $('input[name="siteseo_options[search_engine_google]"]:checked').val() || $('input[name="siteseo_options[search_engine_bring]"]:checked').val(),
			...
			success: function(response){
				if(response.success){
					...
					if(response.data.details.google){
						...
						response.data.details.google.forEach(function(result){
							responseHtml += `
							<tr>
								<td>
								...
								<td>
									<code>URL: ${result.url} - Status: ${result.status_code}</code>
								</td>
							</tr>`;
						});
