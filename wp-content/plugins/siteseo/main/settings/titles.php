...
class Titles{
    ...
    static function post_types(){
		...
		foreach($post_types as $post_type => $post_arr){
			...
			// Load settings
			$option_post_enable = !empty($post_data['enable']) ? $post_data['enable'] : '';
			...
			echo '<div id="'.esc_attr($post_type).'-types">';
				echo '<div class="siteseo-toggle-cnt">
						...
						<div class="siteseo-toggle-meta '.($option_post_enable ? 'active' : '').'" id="siteseo-toggle-meta-'.esc_attr($post_type).'"></div>
						<span id="siteseo-arrow-icon" class="dashicons dashicons-arrow-left-alt siteseo-arrow-icon"></span>
						<p class="toggle_state_posts" id="toggle_state_'.esc_attr($post_type).'">'.($option_post_enable ? ' Click to hide any SEO metaboxes / columns for this post type' : ' Click to show any SEO metaboxes / columns for this post type').'</p>
						<input type="hidden" name="siteseo_options['.esc_attr($post_type).'][enable]" id="enable_'.esc_attr($post_type).'_toggle" value="'.esc_attr($option_post_enable).'" class="siteseo-suboption-toggle">
					</div>
    ...
    static function taxonomies(){
		...
		foreach($taxonomies as $taxonomy => $_tax){
			...
			// Load settings
			$option_enable_category = !empty($options_tax['enable']) ? $options_tax['enable'] : '';
			...
			echo '<div id="'.esc_attr($taxonomy).'-types"><h3>'.esc_html($_tax->label.'['.$taxonomy.']').'</h3>
				<div class="siteseo-toggle-cnt">
					<div class="siteseo-toggle-meta '.($option_enable_category ? 'active' : '').'" id="siteseo-toggle-meta-category"></div>
					<span id="siteseo-arrow-icon" class="dashicons dashicons-arrow-left-alt siteseo-arrow-icon"></span>
					<p class="toggle_state_category" id="toggle_state_category">' .($option_enable_category ? 'Disable' : 'Enable'). '</p>
					<input type="hidden" name="siteseo_options['.esc_attr($taxonomy).'][enable]" id="enable_'.esc_attr($taxonomy).'" value="'.esc_attr($option_enable_category).'" class="siteseo-suboption-toggle">
				</div>
    ...
    static function save_settings(){
        ...
        if(isset($_POST['siteseo_options']['post_types_tab'])){
			...
			foreach($post_types as $post_type){
				$options['titles_single_titles'][$post_type]['enable'] = isset($_POST['siteseo_options'][$post_type]['enable']) ? sanitize_text_field(wp_unslash($_POST['siteseo_options'][$post_type]['enable'])) : '';
        ...
        if(isset($_POST['siteseo_options']['taxonomies_tab'])){
			...
			foreach($taxonomies as $taxonomy){
				$options['titles_tax_titles'][$taxonomy]['enable'] = isset($_POST['siteseo_options'][$taxonomy]['enable']) ? sanitize_text_field(wp_unslash($_POST['siteseo_options'][$taxonomy]['enable'])) : '';
