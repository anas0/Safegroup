...
<div class="product-thumbnails thumbnails">
	...
	if ( $attachment_id ) {
		...
		if ( $image_thumb_link ) {
			$html .= apply_filters( 'woocommerce_single_product_image_thumbnail_html', '<div class="img-thumbnail"><img class="woocommerce-main-thumb img-responsive" alt="' . esc_attr( $image_alt ) . '" src="' . esc_url( $image_thumb_link[0] ) . '" width="' . esc_attr( $image_thumb_link[1] ) . '" height="' . esc_attr( $image_thumb_link[2] ) . '" /></div>', $attachment_id, $post->ID, '' ); // phpcs:disable WordPress.XSS.EscapeOutput.OutputNotEscaped
		}
	...
	if ( $attachment_ids ) {
		foreach ( $attachment_ids as $key => $attachment_id ) {
			...
			if ( isset( $image_thumb_link[0] ) ) {
				...
				$html .= apply_filters( 'woocommerce_single_product_image_thumbnail_html', '<div class="img-thumbnail"><img class="img-responsive" alt="' . esc_attr( $image_alt ) . '" src="' . esc_url( $image_thumb_link[0] ) . '" width="' . esc_attr( $image_thumb_link[1] ) . '" height="' . esc_attr( $image_thumb_link[1] ) . '" /></div>', $attachment_id, $post->ID, '' ); // PHPCS:Ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
		}
	}
