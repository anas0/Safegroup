		...
		if ( ! ( empty( $popup_options ) && empty( get_post_meta( $popup_id, 'popup_animation', true ) ) ) ) {
			...
			if ( ! $return ) {
				...
				if ( ! empty( $popup_options['enable_exit_intent'] ) ) {
					...
					<?php
					$html  = '<div data-trigger-id="popup-builder"' . ( ! empty( $popup_options['disable_overlay' ] ) ? ' data-disable-overlay="true"' : '' ) . ' data-extra-class="popup-builder " data-type="inline" class="porto-exit-intent" data-popup-id="' . $popup_id . '" data-overlay-class="' . esc_attr( $popup_options['animation'] ) . '"' . ( isset( $popup_options['popup_period'] ) ? ' data-expired="' . $popup_options['popup_period'] . '"' : '' ) . '></div>';
				} else {
					$html  = '<div data-trigger-id="popup-builder"' . ( ! empty( $popup_options['disable_overlay' ] ) ? ' data-disable-overlay="true"' : '' ) . ' data-extra-class="popup-builder " data-type="inline" class="porto-modal-trigger porto-onload" data-popup-id="' . $popup_id . '" data-overlay-class="' . esc_attr( $popup_options['animation'] ) . '"' . ( isset( $popup_options['load_duration'] ) ? '  data-timeout="' . $popup_options['load_duration'] . '"' : '' ) . ( isset( $popup_options['popup_period'] ) ? ' data-expired="' . $popup_options['popup_period'] . '"' : '' ) . '></div>';
				}
